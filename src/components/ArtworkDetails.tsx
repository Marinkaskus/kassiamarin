  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="flex flex-col max-w-5xl w-full md:h-auto overflow-hidden bg-white border-0 shadow-lg p-0">
        <div className="w-full flex flex-col">
          {/* Image Container */}
          <div className="relative w-full flex items-center justify-center p-4 md:p-6" onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd}>
            {imageError ? <div className="w-full h-full flex flex-col items-center justify-center py-16">
                ...
                <img ref={imgRef} src={displayImage} alt={currentArtwork.title} className="max-w-full max-h-[75vh] w-auto h-auto object-contain mx-auto" onError={handleImageError} onLoad={handleImageLoad} loading="eager" decoding="async" style={{
            ...
            {allArtworks.length > 1 && currentArtworkIndex !== -1 && <div className="absolute top-1/2 -translate-y-1/2 w-full flex justify-between px-2 md:px-4">
                <button onClick={goToPrevArtwork} className="h-8 w-8 md:h-12 md:w-12 rounded-full bg-foreground/10 backdrop-blur-sm border border-foreground/10 flex items-center justify-center text-foreground hover:bg-foreground/20 transform -translate-x-3 md:-translate-x-6" aria-label="Previous artwork">
                  <ChevronLeft className="h-4 w-4 md:h-6 md:w-6" />
                </button>
                
                <button onClick={goToNextArtwork} className="h-8 w-8 md:h-12 md:w-12 rounded-full bg-foreground/10 backdrop-blur-sm border border-foreground/10 flex items-center justify-center text-foreground hover:bg-foreground/20 transform translate-x-3 md:translate-x-6" aria-label="Next artwork">
                  <ChevronRight className="h-4 w-4 md:h-6 md:w-6" />
                </button>
              </div>}
          </div>
          ...
          <div className="px-6 pb-6 w-full bg-white">
            ...
            <div className="flex flex-col md:flex-row md:items-start md:justify-between max-w-3xl mx-auto">
              ...
              <div className="mt-2 md:mt-0 md:text-right">
                <div className="inline-flex flex-wrap items-center gap-x-2 text-sm text-muted-foreground">
            ...
            {currentArtwork.description && <p className="text-sm text-foreground/80 mt-4 max-w-3xl mx-auto">
